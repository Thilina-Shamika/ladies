...
interface LilianNixonLibraryACF {
  subheading?: string;
  heading?: string;
  cover?: {
    url: string;
...
export default async function LilianNixonLibraryPage() {
  const pageData = await getPage('lilian-nixon-library');
  const acf = (pageData?.acf || {}) as LilianNixonLibraryACF;
      ...
      <section className="relative min-h-[50vh] flex items-center justify-center bg-gray-900">
        <div className="absolute inset-0 w-full h-full z-0">
          {acf.cover?.url && (
            <Image
              src={acf.cover.url}
              alt={acf.cover.alt || acf.heading || 'Lilian Nixon Library'}
        ...
        <div className="relative z-10 flex flex-col items-center justify-center text-center px-4 py-12 w-full">
          ...
          <h1 className="text-4xl md:text-7xl text-white mb-8 drop-shadow-lg font-light">
            {acf.heading ?? 'Lilian Nixon Library'}
          </h1>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          ...
          {acf.content_subheading && (
            <div className="max-w-4xl mx-auto mb-4">
              <div className="text-[#9d0202] text-sm md:text-base font-semibold tracking-widest uppercase">
                {acf.content_subheading}
              </div>
            </div>
          )}
