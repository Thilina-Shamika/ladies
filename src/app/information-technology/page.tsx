...
interface InformationTechnologyACF {
  subheading?: string;
  heading?: string;
  cover?: {
    url: string;
...
export default async function InformationTechnologyPage() {
  const pageData = await getPage('information-technology');
  const acf = (pageData?.acf || {}) as InformationTechnologyACF;
      ...
      <section className="relative min-h-[50vh] flex items-center justify-center bg-gray-900">
        <div className="absolute inset-0 w-full h-full z-0">
          {acf.cover?.url && (
            <Image
              src={acf.cover.url}
              alt={acf.cover.alt || acf.heading || 'Information Technology'}
        ...
        <div className="relative z-10 flex flex-col items-center justify-center text-center px-4 py-12 w-full">
          ...
          <h1 className="text-4xl md:text-7xl text-white mb-8 drop-shadow-lg font-light">
            {acf.heading ?? 'Information Technology'}
          </h1>
        </div>
      </section>
