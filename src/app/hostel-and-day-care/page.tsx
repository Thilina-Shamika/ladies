...
interface HostelAndDayCareACF {
  ...
  block_quote_2?: string;
}

export default async function HostelAndDayCarePage() {
  const pageData = await getPage('hostel-and-day-care');
  const acf = (pageData?.acf || {}) as HostelAndDayCareACF;
      ...
      <section className="relative min-h-[50vh] flex items-center justify-center bg-gray-900">
        <div className="absolute inset-0 w-full h-full z-0">
          {acf.cover?.url && (
            <Image
              src={acf.cover.url}
              alt={acf.cover.alt || acf.heading || 'Hostel and Day Care'}
        ...
        <div className="container mx-auto px-4">
          ...
          {acf.block_quote_2 && (
            <div className="max-w-4xl mx-auto">
              <div className="relative border-4 border-[#9d0202] bg-gray-50/50 p-8 rounded-lg">
                <div
                  className="wysiwyg-content"
                  dangerouslySetInnerHTML={{ __html: acf.block_quote_2 }}
                />
              </div>
            </div>
          )}
