...
interface ScienceLaboratoryACF {
  subheading?: string;
  heading?: string;
  cover?: {
    url: string;
...
export default async function ScienceLaboratoryPage() {
  const pageData = await getPage('science-laboratory');
  const acf = (pageData?.acf || {}) as ScienceLaboratoryACF;
      ...
      <section className="relative min-h-[50vh] flex items-center justify-center bg-gray-900">
        <div className="absolute inset-0 w-full h-full z-0">
          {acf.cover?.url && (
            <Image
              src={acf.cover.url}
              alt={acf.cover.alt || acf.heading || 'Science Laboratory'}
        ...
        <div className="relative z-10 flex flex-col items-center justify-center text-center px-4 py-12 w-full">
          ...
          <h1 className="text-4xl md:text-7xl text-white mb-8 drop-shadow-lg font-light">
            {acf.heading ?? 'Science Laboratory'}
          </h1>
        </div>
      </section>
